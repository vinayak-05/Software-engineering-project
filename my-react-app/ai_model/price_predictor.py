...
def predict_price(crop, location):
    ...
    try:
        ...
        if crop not in unique_crops:
            print(f"Error: Crop '{crop}' not found in dataset", file=sys.stderr)
        ...
        if not os.path.exists(model_path):
            ...
            if not os.path.exists(model_path):
                print("Error: Failed to create model", file=sys.stderr)
                print("0.00", flush=True)
                return
        ...
        # Print only the number to stdout (no formatting, just the raw value)
        print(f"{prediction:.2f}", flush=True)
